...
N <- 40; # number of grid cells per side
...
lambda <- 100  # rate of litter transfer. Lower lambda means more dispersal
...
for (i in 1:j){
  ...
  rootingKernelNF[[i]] <- root/sum(root)*rMaxNF[i] # rMax is the benefit multiplier of larger roots
...
for (i in 1:k){
  ...
  rootingKernelF[[i]] <- root/sum(root)*rMaxF[i]
  lit <- outer(x,y, function(x,y){
...
IN <- 0.7 # g N/m^2/y input of N 
IP <- 0.07 # g P/m^2/y abiotic input of P
